...
<head>
    ...
    <script>
        var safariPark = angular.module('safariPark', ['angularMoment', 'angular-storage']);

        safariPark.filter('sce', ['$sce', function ($sce) {
            return $sce.trustAsHtml;
        }]);

        safariPark.directive('parkDirective', function () {
            return {
                require: 'ngModel'
            }
        });

        safariPark.controller('jsonCtrl', function ($scope, $http, $filter, moment, store) {
            $scope.totalPrice = 0;
            $scope.totalSafari = [];
            $scope.totalEntry = [];
            $scope.todayDate = '';
            ...
            $scope.dateList = [];
            $scope.entrySchedule = [];
            $scope.safariSchedule = [];
            $scope.entryTickets = [];
            $scope.safariTickets = [];
            $scope.entryVisitDate = {};
            $scope.safariVisitDate = {};
            $scope.userTicket = {};
            $scope.warning = '';
            $scope.checkout = false;
            $scope.entryTicketsCheck = [];
            $scope.safariTicketsCheck = [];
            $scope.paymentURI = '';
            $scope.entryTravleDate = '';
            $scope.entryTicketValid = '';
            $scope.entryTimeName = '';
            $scope.entryTimeID = '';
            $scope.safariTravleDate = '';
            $scope.safariTicketValid = '';
            $scope.safariTimeName = '';
            $scope.safariTimeID = '';
            $scope.totalEntryCheck = 0;
            $scope.totalSafariCheck = 0;
            $scope.purchaseVat = '';
            $scope.totalPriceCheck = 0;
            $scope.totalEntryPrice = 0;
            $scope.totalSafariPrice = 0;
            $scope.subTotal = 0;
            ...
            $scope.RegisterUserEmail = '';
            $scope.RegisterUserPhone = '';
            $scope.RegisterPassWord = '';
            $scope.RegisterPassWordRe = '';
            ...
            $scope.UserPass = {};
            $scope.UserPassWarning = '';
            $scope.UserPassSuccess = '';


            var userToken = store.get('userTokenEnc');
            if (userToken !== null) {
                $scope.userState = true;
                $scope.userTokenData = userToken;
            }

            var appDom = '<?php echo APP_DOM;?>';
            $http.get(appDom + 'api/v2/tickets', {headers: {'X-Auth-Ibn': '<?php echo $headerToken; ?>'}}).then(function (data) {
                if (data.data) {
                    $scope.tickets = data.data;
                    $scope.entryTickets = $scope.tickets.entry_tickets;
                    $scope.safariTickets = $scope.tickets.safari_tickets;

                    //DateTime
                    $scope.todayDate = moment().format("DD-MM-YYYY");
                    $scope.nextDate = moment($scope.todayDate, "DD-MM-YYYY").add($scope.tickets.advance_date, 'days').format("DD-MM-YYYY");
                    $scope.visitDate = $scope.todayDate;
                    var dateKey = moment($scope.todayDate, "DD-MM-YYYY").format("DD/MM/YYYY");
                    //DateTime


                    for (var k = 0; k < $scope.tickets.advance_date; k++) {
                        var makedate = moment($scope.todayDate, "DD-MM-YYYY").add(k, 'days').format("DD-MM-YYYY");
                        $scope.dateList.push(makedate);
                    }

                    //Entry Shifts
                    if ($scope.tickets.schedules.entry_tickets.dates[dateKey]) {
                        if ($scope.tickets.schedules.entry_tickets.dates[dateKey] == 'closed') {
                            $scope.entrySchedule = [];
                        } else {
                            var checkEntrySchedule = $scope.tickets.schedules.entry_tickets.dates[dateKey].shifts;
                            if (checkEntrySchedule.length > 0) {
                                $scope.entrySchedule = $scope.tickets.schedules.entry_tickets.dates[dateKey].shifts;
                            } else {
                                var NewObject = [];
                                NewObject.push({
                                    id: '1',
                                    name: 'সারাদিন',
                                    start: $scope.tickets.schedules.entry_tickets.dates[dateKey].open,
                                    end: $scope.tickets.schedules.entry_tickets.dates[dateKey].close,
                                    tickets: $scope.tickets.schedules.entry_tickets.dates[dateKey].tickets
                                });
                                $scope.entrySchedule = NewObject;
                            }
                        }
                    }
                    //Entry Shifts
                    ...
                    //Safari Shifts
                    if ($scope.tickets.schedules.safari_tickets.dates[dateKey]) {
                        if ($scope.tickets.schedules.safari_tickets.dates[dateKey] == 'closed') {
                            $scope.safariSchedule = [];
                        } else {
                            var checkSafariSchedule = $scope.tickets.schedules.safari_tickets.dates[dateKey].shifts;
                            if (checkSafariSchedule.length > 0) {
                                $scope.safariSchedule = $scope.tickets.schedules.safari_tickets.dates[dateKey].shifts;
                            } else {
                                var NewObject = [];
                                NewObject.push({
                                    id: '1',
                                    name: 'সারাদিন',
                                    start: $scope.tickets.schedules.safari_tickets.dates[dateKey].open,
                                    end: $scope.tickets.schedules.safari_tickets.dates[dateKey].close,
                                    tickets: $scope.tickets.schedules.safari_tickets.dates[dateKey].tickets
                                });

                                $scope.safariSchedule = NewObject;
                            }
                        }
                    }
                    //Safari Shifts
                    ...
                    var entryNum = $scope.entryTickets.length;

                    for (var i = 0; i < entryNum; i++) {
                        var enID = $scope.entryTickets[i].slug;
                        $scope.totalEntry[enID] = [];
                        $scope.totalEntry[enID] = {"total": 0};
                    }


                    var safariNum = $scope.safariTickets.length;

                    for (var i = 0; i < safariNum; i++) {
                        var enID = $scope.safariTickets[i].slug;
                        $scope.totalSafari[enID] = [];
                        $scope.totalSafari[enID] = {"total": 0};
                    }
            ...
            $scope.entryTicket = function (ticketIndex, ticketID, ticketPrice, ticketNum, ticketName, ticketRequire) {
                var price = parseFloat(ticketPrice) * parseFloat(ticketNum);
                $scope.totalEntry[ticketIndex] = [];
                $scope.totalEntry[ticketIndex].total = price;
                $scope.totalEntry[ticketIndex].ticketID = ticketID;
                $scope.totalEntry[ticketIndex].ticketPrice = ticketPrice;
                $scope.totalEntry[ticketIndex].ticketNum = ticketNum;
                $scope.totalEntry[ticketIndex].ticketName = ticketName;
                $scope.totalEntry[ticketIndex].ticketRequire = ticketRequire;


                var entrySum = 0;
                var totalEntry = $scope.totalEntry;
                Object.keys(totalEntry).forEach(function (key) {
                    entrySum += parseFloat(totalEntry[key].total);
                });

                var safariSum = 0;
                var totalSafari = $scope.totalSafari;
                Object.keys(totalSafari).forEach(function (key) {
                    safariSum += parseFloat(totalSafari[key].total);
                });


                var total = entrySum + safariSum;
                $scope.totalPrice = total;
            };

            $scope.safariTicket = function (ticketIndex, ticketID, ticketPrice, ticketNum, ticketName) {
                var price = parseFloat(ticketPrice) * parseFloat(ticketNum);
                $scope.totalSafari[ticketIndex] = [];
                $scope.totalSafari[ticketIndex].total = price;
                $scope.totalSafari[ticketIndex].ticketID = ticketID;
                $scope.totalSafari[ticketIndex].ticketPrice = ticketPrice;
                $scope.totalSafari[ticketIndex].ticketNum = ticketNum;
                $scope.totalSafari[ticketIndex].ticketName = ticketName;
            ...
            $scope.getVisitDate = function (date) {
                $scope.visitDate = date;

                var dateKey = moment(date, "DD/MM/YYYY").format("DD/MM/YYYY");
            ...
            $scope.saveEntryShift = function (shidtID) {
                var dateVal = moment(this.visitDate, "DD/MM/YYYY").format("DD/MM/YYYY");
                $scope.entryVisitDate = {date: dateVal, shiftID: shidtID};

            };

            $scope.saveSafariShift = function (shidtID) {
                moment.locale('en');
                var dateVal = moment(this.visitDate, "DD/MM/YYYY").format("DD/MM/YYYY");
                $scope.safariVisitDate = {date: dateVal, shiftID: shidtID};
            };
            ...
            $scope.reviewOrder = function () {
                ...
                var totalEntry = $scope.totalEntry;
                Object.keys(totalEntry).forEach(function (key) {
                    if (totalEntry[key].ticketNum) {
                        if (totalEntry[key].ticketNum > 0) {
                            entryJson[key] = {
                                "ticketID": totalEntry[key].ticketID,
                                "ticketName": totalEntry[key].ticketName,
                                "ticketNum": totalEntry[key].ticketNum,
                                "ticketPrice": totalEntry[key].ticketPrice,
                                "total": totalEntry[key].total
                            };

                            if (totalEntry[key].ticketRequire == 0) {
                                require = 'valid';
                            }
                            entryNum += parseInt(totalEntry[key].ticketNum);
                        }
                    }
                ...
                var totalSafari = $scope.totalSafari;
                Object.keys(totalSafari).forEach(function (key) {
                    if (totalSafari[key].ticketNum) {
                        if (totalSafari[key].ticketNum > 0) {
                            safariJson[key] = {
                                "ticketID": totalSafari[key].ticketID,
                                "ticketName": totalSafari[key].ticketName,
                                "ticketNum": totalSafari[key].ticketNum,
                                "ticketPrice": totalSafari[key].ticketPrice,
                                "total": totalSafari[key].total
                            };
                        }

                        safariNum += parseInt(totalSafari[key].ticketNum);
                    }
                });


                $scope.userTicket = {
                    "entry_tickets": entryJson,
                    "safari_tickets": safariJson,
                    "entryDate": this.entryVisitDate,
                    "safariDate": this.safariVisitDate,
                    "platform": "web"
                };
                ...
                var selectedTime = '';
                if ($scope.entryVisitDate['shiftID']) {
                    var entryDateData = this.entryVisitDate['shiftID'];
                    var dateData = entryDateData.split("|");
                    var timeCheck = dateData[3];
                    selectedTime = moment(todayDate + ' ' + timeCheck, "DD/MM/YYYY hh:mm A").format("X");
                }

                if ($scope.safariVisitDate['shiftID']) {
                    var entryDateData = this.safariVisitDate['shiftID'];
                    var dateData = entryDateData.split("|");
                    var timeCheck = dateData[3];
                    selectedTime = moment(todayDate + ' ' + timeCheck, "DD/MM/YYYY hh:mm A").format("X");
                }
                ...
                if (Object.keys(entryJson).length > 0 && require == 'invalid') {
                    $scope.warning = '<?php echo($lang == 'bn' ? '<span class="bangla">নির্বাচিত টিকেট এর জন্য অভিভাবক প্রয়োজন</span>' : 'You need guardian for selected ticket.'); ?>';
                }
                else if (Object.keys(entryJson).length > 0 && !$scope.entryVisitDate['shiftID']) {
                    $scope.warning = '<?php echo($lang == 'bn' ? '<span class="bangla">তারিখ এবং সময় নির্বাচন করুন</span>' : 'Please select date and time.'); ?>';
                }
                else if (Object.keys(safariJson).length > 0 && !$scope.safariVisitDate['shiftID']) {
                    $scope.warning = '<?php echo($lang == 'bn' ? '<span class="bangla">তারিখ এবং সময় নির্বাচন করুন</span>' : 'Please select date and time.'); ?>';
                }
                else if (this.safariVisitDate['date'] && $scope.safariVisitDate['date'] == todayDate && nowUnix > parseInt(selectedTime) || $scope.entryVisitDate['date'] && $scope.entryVisitDate['date'] == todayDate && nowUnix > parseInt(selectedTime)) {
                    $scope.warning = '<?php echo($lang == 'bn' ? '<span class="bangla">নির্বাচিত সময় ইতিমধ্যে অতিক্রম করেছে</span>' : 'Selected time already passed.'); ?>';
                }
                else if (Object.keys(entryJson).length > 0 || Object.keys(safariJson).length > 0) {

                    if (Object.keys(entryJson).length > 0) {
                        var entryDateData = this.entryVisitDate['shiftID'];
                        var dateData = entryDateData.split("|");
                        if (dateData[2] && entryNum < parseInt(dateData[2])) {
                            $scope.checkout = true;
                            $scope.warning = '';
                        } else {
                            $scope.warning = '<?php echo($lang == 'bn' ? '<span class="bangla">আপনার নির্বাচিত তারিখ এবং সময়ে যথেষ্ট টিকিট নেই</span>' : 'There are not enough tickets on your selected date.'); ?>';
                        }
                    }


                    if (Object.keys(safariJson).length > 0) {
                        var safariDateData = $scope.safariVisitDate['shiftID'];
                        var dateData = safariDateData.split("|");
                        if (dateData[2] && safariNum < parseInt(dateData[2])) {
                            $scope.checkout = true;
                            $scope.warning = '';
                        } else {
                            $scope.warning = '<?php echo($lang == 'bn' ? '<span class="bangla">আপনার নির্বাচিত তারিখ এবং সময়ে যথেষ্ট টিকিট নেই</span>' : 'There are not enough tickets on your selected date.'); ?>';
                    ...
                    $scope.warning = '<?php echo($lang == 'bn' ? '<span class="bangla">আপনি কোন টিকেট নির্বাচন করেননি</span>' : 'You have not selected any tickets.'); ?>';
                }
            };

            $scope.checkOutInit = function () {
                $scope.entryTicketsCheck = $scope.userTicket.entry_tickets;
                $scope.safariTicketsCheck = this.userTicket.safari_tickets;
                var vatPrice = $scope.tickets.fee;
                var validTill = $scope.tickets.valid;
                $scope.paymentURI = $scope.tickets.payment;


                if ($scope.userTicket.entryDate['date']) {
                    var datePick = $scope.userTicket.entryDate['date'];
                    var dateMoment = moment(datePick, "DD/MM/YYYY");
                    var dateAdd = dateMoment.add(validTill, 'days');
                    $scope.entryTravleDate = datePick;
                    $scope.entryTicketValid = dateAdd.format("DD/MM/YYYY");

                    var entryDateData = $scope.userTicket.entryDate['shiftID'];
                    var dateData = entryDateData.split("|");
                    $scope.entryTimeName = dateData[1];
                    $scope.entryTimeID = dateData[0];
                }

                if ($scope.userTicket.safariDate['date']) {
                    var datePick = $scope.userTicket.safariDate['date'];
                    var dateMoment = moment(datePick, "DD/MM/YYYY");
                    var dateAdd = dateMoment.add(validTill, 'days');
                    $scope.safariTravleDate = datePick;
                    $scope.safariTicketValid = dateAdd.format("DD/MM/YYYY");

                    var entryDateData = $scope.userTicket.safariDate['shiftID'];
                    var dateData = entryDateData.split("|");
                    $scope.safariTimeName = dateData[1];
                    $scope.safariTimeID = dateData[0];
                }

                var entryJson = [];
                var totalEntry = $scope.entryTicketsCheck;
                Object.keys(totalEntry).forEach(function (key) {
                    entryJson.push({
                        "ticketID": totalEntry[key].ticketID,
                        "ticketName": totalEntry[key].ticketName,
                ...
                $scope.entryTicketsCheck = entryJson;


                var safariJson = [];
                var totalSafari = $scope.safariTicketsCheck;
                Object.keys(totalSafari).forEach(function (key) {
                    safariJson.push({
                        "ticketID": totalSafari[key].ticketID,
                        "ticketName": totalSafari[key].ticketName,
                ...
                $scope.safariTicketsCheck = safariJson;


                $scope.totalEntryCheck = Object.keys(entryJson).length;
                $scope.totalSafariCheck = Object.keys(safariJson).length;


                var entrySum = 0;
                Object.keys(totalEntry).forEach(function (key) {
                    entrySum += totalEntry[key].total;
                });

                var safariSum = 0;
                Object.keys(totalSafari).forEach(function (key) {
                    safariSum += totalSafari[key].total;
                });

                var total = entrySum + safariSum;
                var vatcalculate = (vatPrice / 100) * total;
                $scope.totalEntryPrice = entrySum;
                $scope.totalSafariPrice = safariSum;
                $scope.subTotal = total;
                $scope.purchaseVat = vatcalculate.toFixed(2);
                $scope.totalPriceCheck = total + vatcalculate;
            };

            $scope.confirmOrder = function () {
                if ($scope.userState) {
                    var entryJson = [];
                    var totalEntry = $scope.userTicket.entry_tickets;
                    Object.keys(totalEntry).forEach(function (key) {
                        entryJson.push({
                            "ticketID": totalEntry[key].ticketID,
                            "ticketQuantity": totalEntry[key].ticketNum
                        });
                    });

                    var safariJson = [];
                    var totalSafari = $scope.userTicket.safari_tickets;
                    Object.keys(totalSafari).forEach(function (key) {
                        safariJson.push({
                            "ticketID": totalSafari[key].ticketID,
                            "ticketQuantity": totalSafari[key].ticketNum
                        });
                    });
                    ...
                    jsonSend.push({
                        "entry_tickets": entryJson,
                        "safari_tickets": safariJson,
                        "entry_travel_date": $scope.entryTravleDate,
                        "entry_valid_date": $scope.entryTicketValid,
                        "entry_timeID": $scope.entryTimeID,
                        "safari_travel_date": $scope.safariTravleDate,
                        "safari_valid_date": $scope.safariTicketValid,
                        "safari_timeID": $scope.safariTimeID,
                        "platform": "web"
                    });

                    $scope.loader = true;

                    $http.get($scope.paymentURI + $scope.userTokenData + '/' + JSON.stringify(jsonSend)).then(function (data) {

                        if (data.data.Status) {
                            if (data.data.Status == '407') {
                                $scope.checkoutWarning = data.data.Message;
                            } else if (data.data.Status == '408') {
                                $scope.checkoutWarning = '';
                            ...
                            $scope.checkoutWarning = '<?php echo($lang == 'bn' ? '<span class="bangla">আপনার অনুরোধের সাথে সমস্যা হচ্ছে।</span>' : 'Having problems with your request.'); ?>';
                            $scope.loader = false;
                        }
            ...
            $scope.userLogin = function () {
                ...
                $http.post(appDom + 'api/v2/login', {
                    "username": userName,
                    "password": passWord,
                    "platform": "Web",
                    "language": "<?php echo $lang;?>"
                }, {headers: {'X-Auth-Ibn': '<?php echo $headerToken; ?>'}}).then(function (data) {

                    if (data.data.Status) {

                        if (data.data.Status == '407' || data.data.Status == '405') {
                            $scope.LoginWarning = data.data.Message;
                        } else if (data.data.Status == '408') {
                            $scope.LoginWarning = '';
                            $scope.userState = true;
                            store.set('userTokenEnc', data.data.Token);
                            $scope.userTokenData = data.data.Token;
                        ...
                        $scope.LoginWarning = '<?php echo($lang == 'bn' ? '<span class="bangla">আপনার অনুরোধের সাথে সমস্যা হচ্ছে।</span>' : 'Having problems with your request.'); ?>';
                        $scope.loader = false;
                    }
            ...
            $scope.userRegister = function () {
                ...
                var userEmail = $scope.RegisterUserEmail;
                var userPhone = $scope.RegisterUserPhone;
                var passWord = $scope.RegisterPassWord;
                var passWordRe = $scope.RegisterPassWordRe;

                $scope.loader = true;

                $http.post(appDom + 'api/v2/register', {
                    "name": fullName,
                    "email": userEmail,
                    "phone": userPhone,
                    "password": passWord,
                    ...
                    if (data.data.Status) {
                        ...
                        if (data.data.Status == '407') {
                            $scope.RegisterWarning = data.data.Message;
                        } else if (data.data.Status == '408') {
                            $scope.RegisterWarning = '';
            ...
            $scope.userForget = function () {
                var userName = $scope.ForgetUserName;

                $scope.loader = true;

                $http.post(appDom + 'api/v2/forget', {
                    "email": userName,
                    "language": "<?php echo $lang;?>"
                }, {headers: {'X-Auth-Ibn': '<?php echo $headerToken; ?>'}}).then(function (data) {
                    ...
                    if (data.data.Status) {
                        ...
                        if (data.data.Status == '407' || data.data.Status == '405') {
                            $scope.ForgetWarning = data.data.Message;
                        } else if (data.data.Status == '408') {
                            $scope.ForgetWarning = '';
                            $scope.ForgetSuccess = data.data.Message;
                        }
            ...
            $scope.userAccount = function () {
                $http.get(appDom + 'api/v2/account/' + $scope.userTokenData, {headers: {'X-Auth-Ibn': '<?php echo $headerToken; ?>'}}).then(function (data) {
                    if (data.data.account) {
                        $scope.userFullName = data.data.account.name;

                        if (data.data.bookings) {
                            $scope.bookingData = data.data.bookings;
                        }

                    } else {
                        $scope.userState = false;
                        store.remove('userTokenEnc');
                    }

                });
            };

            $scope.bookingDetail = function (bookingID) {
                $scope.singleBooking = $filter('filter')($scope.bookingData, {id: bookingID});
            };

            $scope.bookBack = function () {
                $scope.singleBooking = [];
                $scope.PasswordReset = false;
            };
            ...
            $scope.userPassword = function () {
                var userPass = $scope.UserPass.password;
                var userPassRe = $scope.UserPass.confirm;

                $scope.loader = true;


                $http.post(appDom + 'api/v2/password/' + $scope.userTokenData, {
                    "password": userPass,
                    "re_password": userPassRe,
                    "language": "<?php echo $lang;?>"
                }, {headers: {'X-Auth-Ibn': '<?php echo $headerToken; ?>'}}).then(function (data) {
                    if (data.data.Status) {

                        if (data.data.Status == '407') {
                            $scope.UserPassWarning = data.data.Message;
                        } else if (data.data.Status == '408') {
                            $scope.UserPassWarning = '';
                            $scope.UserPassSuccess = data.data.Message;
                        }

                        $scope.loader = false;

                    } else {
                        $scope.UserPassWarning = '<?php echo($lang == 'bn' ? '<span class="bangla">আপনার অনুরোধের সাথে সমস্যা হচ্ছে।</span>' : 'Having problems with your request.'); ?>';
                        $scope.loader = false;
                    }
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="accountModalLabel">
                        Hello, {{userFullName}}
                        <i ng-click="accountSettings()" title="Settings" class="fa fa-gear"
                           style="font-size: 20px; cursor: pointer; margin: 0 5px"></i>

                        <i ng-click="logOut()" title="Logout" class="fa fa-sign-out"
                           style="font-size: 20px; cursor: pointer; margin: 0 5px"></i>
                    </h4>
                </div>
                <div class="modal-body">
