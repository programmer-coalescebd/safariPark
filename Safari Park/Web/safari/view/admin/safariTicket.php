...
$orders = $DB->sql("SELECT order_id, unique_id, invoice_id, travel_date, phone_number, pnr_number, ticket_amount, `time`, status FROM `orders` WHERE ticket_category = 'safariTicket' " . $addSQL . " ORDER BY order_id DESC", 0, 1000, 0);

$sql = "SELECT";
$sql .= "(SELECT COUNT(order_id) FROM orders WHERE ticket_category = 'safariTicket' " . $addSQL . ") AS orders,";
$sql .= "(SELECT SUM(total_ticket) FROM orders WHERE ticket_category = 'safariTicket' " . $addSQL . ") AS tickets,";
$sql .= "(SELECT SUM(total_ticket) FROM orders WHERE ticket_category = 'safariTicket' AND visit_date != '' " . $addSQL . ") AS visits,";
//$addSQL = preg_replace('/ AND status = "\d"/', "", $addSQL);
//$sql .= "(SELECT SUM(ticket_amount) FROM orders WHERE ticket_category = 'safariTicket' AND status = '5' " . $addSQL . ") AS amount";
$sql .= "(SELECT SUM(ticket_amount) FROM orders WHERE ticket_category = 'safariTicket' " . $addSQL . ") AS amount";
...
?>
    ...
    <div class="row">
        <h4 class="m-t-0 header-title"><?php echo($lang == 'bn' ? '<span class="bangla">' . $lang_ticket_safari_list[$lang] . '</span>' : $lang_ticket_safari_list[$lang]); ?></h4>
