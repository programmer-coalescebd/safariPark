                    ...
                    <div class="col-sm-12">
                        <div class="col-sm-11">
                            <input type="number"
                                   ...
                                   name="secret"
                                   placeholder="<?php echo $lang_ticket_pnrNumber[$lang]; ?>" <?php echo(isset($_POST["secret"]) ? 'value="' . $_POST["secret"] . '"' : ''); ?>>
                        </div>
                        <div class="col-sm-1">
                            <button type="submit"
...
<div class="row" style="margin-bottom: 100px !important;">
    <div class="col-lg-12">
        ...
        <div class="m-b-20" <?php echo(!isset($_POST["secret"]) ? 'style="display:none"' : ''); ?>>
            <?php
            if (isset($_POST["secret"])) {
                $secret = $_POST["secret"];

                $ticketSearch = $DB->sql("SELECT * FROM `orders` WHERE pnr_number = '" . $secret . "'", 0, 0, 0);

                if (count($ticketSearch) > 0) {
                    $ticketSearch = $ticketSearch[0];
                    $order = $ticketSearch;
                    $job = $ticketSearch["unique_id"];
                    $userData = $DB->sql("SELECT unique_id, email, first_name FROM `user` WHERE user_id = '" . $ticketSearch["user_id"] . "'", 0, 0, 0);
                    if (count($userData) > 0) {
                        $userData = $userData[0];
                    }
                    ?>
                            ...
                            <div class="col-sm-12 parentTab" style="margin-bottom: 20px">
                                ...
                                if ($order["status"] == 5) {
                                    echo '<button type="button" class="btn btn-danger btn-bordered"
                                            onclick="location.href=\'/gate/cancelTicket/' . $job . '\';" style="position:absolute;top:-5px;right:120px">
                                        ' . ($lang == 'bn' ? '<span class="bangla">' . $lang_order_report[$lang] . '</span>' : $lang_order_report[$lang]) . '
                                            ...
                                            onclick="location.href=\'/gate/claimTicket/' . $job . '\';" style="position:absolute;top:-5px;right:10px">
                                        ...
                                        <div class="col-lg-3 col-sm-6">
                                            <div class="widget-inline-box text-center">
                                                <h4><i class="text-primary mdi mdi-receipt"></i> <b
                                                            data-plugin="counterup"><?php echo $order["invoice_id"]; ?></b>
                                                </h4>
                                        ...
                                        <div class="col-lg-3 col-sm-6">
                                            <div class="widget-inline-box text-center">
                                                <h4><i class="text-info mdi mdi-lock"></i> <b
                                                            data-plugin="counterup"><?php echo $order["pnr_number"]; ?></b>
                                                </h4>
                    ...
                    echo '<h2 align="center"><span ' . ($lang == 'bn' ? 'class="bangla"' : '') . '>' . $lang_nothing_found[$lang] . '</span></h2>';
                }
            }
            ?>
        </div>
